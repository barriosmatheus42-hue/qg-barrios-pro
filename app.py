...
st.set_page_config(page_title="QG Barrios PRO v2", layout="wide")
...
ARQUIVO_BANCO = "banco_barrios_pro_v2.json"
...
# ==========================================
# 1. MOTOR MATEMÁTICO (POISSON + xG + DECAIMENTO)
# ==========================================

def calcular_poisson(media_casa, media_fora):
    """Matriz de Poisson 6x6 baseada na 'Super Média' (Gols + xG)"""
...
# ==========================================
# 2. BUSCAS DE API E LÓGICA DE DADOS
# ==========================================

def calcular_pseudo_xg(stats):
    """Calcula xG baseado em chutes no alvo e chances reais se a API não entregar xG direto"""
    sog = 0
    big_chances = 0
    for s in stats:
...
def buscar_stats_partida(fixture_id, team_id):
    """Busca xG real ou dados para Pseudo-xG de uma partida específica"""
...
def buscar_historico_global(team_id, current_league_id, last_n=12): 
    """Nova lógica: Trava de 6 meses, Time Decay e xG Weighting"""
    url = f"{BASE_URL}/fixtures"
    params = {'team': team_id, 'last': 25, 'status': 'FT'} # Puxamos 25 para filtrar por data
    try:
        res = requests.get(url, headers=HEADERS, params=params).json()
        ...
        # Se tiver poucos jogos em 6 meses, pegamos os últimos 8 independente da data (segurança)
        if len(jogos_validos) < 5:
            jogos_validos = res['response'][:8]
        else:
            jogos_validos = jogos_validos[:last_n]
        ...
        for idx, j in enumerate(jogos_validos):
            f_id = j['fixture']['id']
            # Time Decay: Jogos mais recentes valem mais (Exponencial)
            dias_atras = (hoje - datetime.datetime.strptime(j['fixture']['date'][:10], '%Y-%m-%d')).days
            peso_tempo = math.exp(-0.005 * dias_atras) 
            
            # Peso de Competição: Se for a mesma liga do jogo de hoje, peso extra
            ...
            # Buscar xG da partida (Isso consome créditos, então fazemos apenas para os últimos 6)
            xg_f, xg_s = 0, 0
            if idx < 6: 
            ...
            total_xg_f += (xg_f or gf) * peso_final # Se não tiver xG, usa o gol real
        ...
        return {
            "media_gols_f": total_gols_f / soma_pesos,
            "media_gols_s": total_gols_s / soma_pesos,
...
# [Manter funções buscar_odds_vips, buscar_standings, buscar_h2h originais]
        ...
        if f_id not in banco_local["datas"][data_str]["stats"]:
            ...
            if s_h and s_a: 
                banco_local["datas"][data_str]["stats"][f_id] = {
                    ...
                    "h": s_h, "a": s_a, "standings": standings, "h2h": h2h
                }
...
def calcular_ranking_dinamico(f_id, data_str, mercado_alvo, mercados_ativos):
    ...
    # Super Média Ponderada: 60% xG + 40% Gols Reais
    m_h = (dados['h']['media_xg_f'] * 0.6 + dados['h']['media_gols_f'] * 0.4 + dados['a']['media_xg_s'] * 0.6 + dados['a']['media_gols_s'] * 0.4) / 2
    m_a = (dados['a']['media_xg_f'] * 0.6 + dados['a']['media_gols_f'] * 0.4 + dados['h']['media_xg_s'] * 0.6 + dados['h']['media_gols_s'] * 0.4) / 2
    ...
    if mercado_alvo == "🛡️ Zagueiros":
        ...
        return max(evs) if evs else -999
    elif mercado_alvo in mercados_ativos: return get_ev(dados, p, mercado_alvo)
    return -999
...
def renderizar_mercado(col, titulo, p_dict, key, odds_dict):
    ...
    badge = f'<div style="color:#28a745; font-size:10px; font-weight:bold;">VALOR +{ev:.1f}%</div>' if ev > 3 else ''
    estilo = "border:1px solid #28a745; background-color:#1a2b1f;" if ev > 3 else "border:1px solid #333; background-color:#111;"
    html = f'<div style="{estilo} padding:6px; border-radius:6px; text-align:center; margin-bottom:5px;"><div style="font-size:9px; color:#aaa;">{titulo}</div><div style="font-size:14px; font-weight:bold;">{prob:.0f}%</div><div style="font-size:10px;">O: {casa}</div>{badge}</div>'
    with col: st.markdown(html, unsafe_allow_html=True)

# ==========================================
# 4. INTERFACE E FIX BUG DE CÓPIA
# ==========================================

with st.sidebar:
    st.markdown("## 👑 QG Barrios PRO v2")
    data_consulta = st.date_input("Data", datetime.date.today())
    ...
    todos_mercados = {"HOME": "Casa", "DRAW": "Empate", "AWAY": "Fora", "1X": "1X", "X2": "X2", "BTTS": "BTTS", "OVER": "Over 2.5", "UNDER": "Under 2.5"}
    mercados_ativos = st.multiselect("Mercados:", list(todos_mercados.keys()), default=list(todos_mercados.keys()))
    
    mercado_filtro = st.selectbox("Ordenar por:", ["Geral", "🛡️ Zagueiros", "🎯 Meio-Campo", "🚀 Atacantes"] + list(todos_mercados.keys()))
...
if agenda:
    ...
    if st.button(f"🚀 Analisar ({len(jogos_visiveis)})", type="primary", use_container_width=True): acao_analisar(jogos_visiveis, data_str)

    for j in jogos_visiveis:
        f_id = str(j['fixture']['id']); d = banco_local["datas"][data_str]["stats"].get(f_id)
        st.markdown(f'<div style="border:1px solid #333; border-radius:8px; padding:10px; background-color:#0e1117; margin-bottom:10px;"><b>{j["teams"]["home"]["name"]} x {j["teams"]["away"]["name"]}</b> <small>({j["league"]["name"]})</small></div>', unsafe_allow_html=True)
        
        if d and "h" in d:
            m_h = (d['h']['media_xg_f'] * 0.6 + d['h']['media_gols_f'] * 0.4 + d['a']['media_xg_s'] * 0.6 + d['a']['media_gols_s'] * 0.4) / 2
            m_a = (d['a']['media_xg_f'] * 0.6 + d['a']['media_gols_f'] * 0.4 + d['h']['media_xg_s'] * 0.6 + d['h']['media_gols_s'] * 0.4) / 2
            p = calcular_poisson(m_h, m_a)
            
            if p:
                cols = st.columns(len(mercados_ativos))
                for idx, mk in enumerate(mercados_ativos): renderizar_mercado(cols[idx], todos_mercados[mk], p, mk, d['odds'])
                
                with st.expander("📊 Raio-X Detalhado"):
                    col1, col2 = st.columns(2)
                    with col1:
                        st.write(f"**🏠 {j['teams']['home']['name']}**")
                        st.write(f"xG Pró: {d['h']['media_xg_f']:.2f} | Gols: {d['h']['media_gols_f']:.2f}")
                    with col2:
                        st.write(f"**🚀 {j['teams']['away']['name']}**")
                        st.write(f"xG Pró: {d['a']['media_xg_f']:.2f} | Gols: {d['a']['media_gols_f']:.2f}")
                    
                    # BUG FIX: Botão de cópia simplificado para evitar flutuações
                    resumo = f"Jogo: {j['teams']['home']['name']} x {j['teams']['away']['name']}\nProbabilidades:\n"
                    for mk in mercados_ativos: resumo += f"- {mk}: {p[mk]['prob']:.0f}% | EV: {get_ev(d, p, mk):.1f}%\n"
                    st.text_area("Copiar para Gemini:", resumo, height=100)
                    
            if st.button("🔄 Atualizar", key=f"upd_{f_id}"): acao_analisar([j], data_str, force=True)
        else: st.caption("Aguardando análise...")
