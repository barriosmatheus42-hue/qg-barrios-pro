...
def buscar_historico_global(team_id, last_n=20): 
    ...
    try:
        ...
        if res.get('response'):
            ...
            return {"media_feita": gols_feitos / total, "media_sofrida": gols_sofridos / total, "total_jogos": total, "detalhe_ligas": competicoes, "forma": "".join(forma[::-1])}
    except: return None
    return None
...
def acao_analisar(jogos_alvo, data_str, force=False):
    ...
    for idx, jogo in enumerate(jogos_alvo):
        ...
        if f_id not in banco_local["datas"][data_str]["stats"]:
            ...
            odds = buscar_odds_vips(f_id); s_h, s_a = buscar_historico_global(h_id), buscar_historico_global(a_id)
