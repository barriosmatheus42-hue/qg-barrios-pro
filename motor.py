...
def calibrar_liga(
    ...
    # params.calibradores["1X2_HOME"] para uso direto em produção.
    _treinar_cal_1x2_home(df, params_final)

    return params_final


def _treinar_cal_1x2_home(
    df_jogos: "pd.DataFrame",
    params: "ParametrosLiga",
    n_min: int = 40,
) -> None:
    """
    Treina calibrador isotônico para o mercado HOME (1X2) usando dados em-amostra.

    Chamado ao final de calibrar_liga(). Utiliza os mesmos jogos que treinaram o
    D-C: para cada jogo, calcula a probabilidade bruta HOME via prever_jogo() e
    compara com o resultado real (home_goals > away_goals = vitória em casa).

    O calibrador corrige a superestimação nativa do D-C em HOME, validada no
    laboratório walk-forward: ROI HOME sobe de +2.6% (RAW) para +8.8% (calibrado).
    Resultado armazenado em params.calibradores["1X2_HOME"].

    Fallback silencioso se sklearn ausente, dados insuficientes ou prever_jogo falhar.
    """
    if not _SKLEARN_OK:
        return
    ...
    for _, row in df_jogos.iterrows():
        ...
        if h_id not in params.times or a_id not in params.times:
            continue
        ...
        h_goals = float(row.get("home_goals", 0) or 0)
        a_goals = float(row.get("away_goals", 0) or 0)
        probs.append(prev["mercados"]["HOME"] / 100.0)
        labels.append(1 if h_goals > a_goals else 0)

    if len(probs) < n_min:
        return

    cal = MarketCalibrator(mercado="1X2_HOME")
    cal.treinar(probs, labels)
    if cal.n_amostras >= n_min:
        params.calibradores["1X2_HOME"] = cal
...
def calcular_overround_1x2(odd_h: float, odd_d: float, odd_a: float) -> float:
    """
    Calcula o overround real do mercado 1X2 a partir das três odds.

    overround = sum(1/odd_i) — tipicamente 1.04 a 1.10 (4-10% de margem).

    Por que usar isso em vez de MARGEM_BK=1.05 fixo?
    Cada jogo/liga tem margem diferente. Passar o overround real a
    comparar_com_mercado() produz prob_mercado_pct e divergencia_pp
    mais precisos, sem qualquer suposição paramétrica por liga.

    Uso portável: backtest lê H/D/A de CSV; produção lê de API.
    A função é idêntica em ambos os contextos.

    Retorna 1.0 se alguma odd for inválida (fallback neutro).
    """
    if any(o <= 1.0 for o in (odd_h, odd_d, odd_a)):
        return 1.0
    return 1.0 / odd_h + 1.0 / odd_d + 1.0 / odd_a
...
# =========================================================================
# 11. CALIBRAÇÃO ADAPTATIVA PARA COMPETIÇÕES DE SELEÇÕES
# =========================================================================

# Mapeamento ranking FIFA → força ofensiva/defensiva esperada.
# Tupla: (ranking_max_incluso, alpha_prior, beta_prior)
# alpha > 1 = ataque forte | beta < 1 = defesa forte
_STRENGTH_TIERS_FIFA: list = [
    (5,   1.60, 0.65),   # Elite: BRA, ARG, FRA, ENG, ESP...
    (15,  1.40, 0.75),   # Top 6-15
    (30,  1.20, 0.88),   # Top 16-30
    (50,  1.00, 1.00),   # Mid-table (neutro)
    (80,  0.82, 1.18),   # Abaixo da média
    (999, 0.65, 1.35),   # Fraco
]


def _prior_por_ranking(ranking: int) -> tuple:
    """Retorna (alpha_prior, beta_prior) para um ranking FIFA."""
    for rank_max, alpha, beta in _STRENGTH_TIERS_FIFA:
        if ranking <= rank_max:
            return alpha, beta
    return 0.65, 1.35


def calibrar_copa_mundo(
    df_jogos: pd.DataFrame,
    league_id: int,
    season: int,
    xi: float = 0.003,
    data_referencia: Optional[dt.datetime] = None,
    min_aparicoes_time: int = 2,
    max_times: int = 64,
    fifa_rankings: Optional[dict] = None,
    forca_prior: float = 15.0,
    nomes_times: Optional[dict] = None,
    seasons_incluidas: Optional[list] = None,
    peso_xg: float = 0.0,
) -> ParametrosLiga:
    """
    Calibra D-C para competições de seleções (Copa do Mundo, Eliminatórias, Copa América).

    Diferenças em relação a calibrar_liga():
    - xi=0.003: decay mais agressivo — peso 0.5 em ~231 dias. Prioriza eliminatórias
      recentes sobre amistosos de 2 anos atrás.
    - min_aparicoes_time=2: seleções jogam poucos jogos — aceita com apenas 2 aparições.
    - gamma limitado a [0.9, 1.3]: vantagem de campo menor em estádio neutro (WC).
    - Prior Bayesiano via fifa_rankings {team_id: ranking_int}: estabiliza alpha/beta
      de seleções com amostra pequena puxando em direção à força esperada pelo ranking.
      Se None, usa prior neutro (alpha=beta=1) com forca_prior reduzida.

    Interface idêntica a calibrar_liga() para substituição transparente no DadosManager.
    """
    if data_referencia is None:
        data_referencia = dt.datetime.now()
    ...
    # Filtro de seleções raras — mais permissivo que ligas domésticas
    aparicoes = pd.concat([df["home_id"], df["away_id"]]).value_counts()
    times_validos = set(aparicoes[aparicoes >= min_aparicoes_time].index)
    if len(times_validos) > max_times:
        times_validos = set(aparicoes.nlargest(max_times).index)
    ...
    if len(df) < 8:
        raise ValueError(
            f"Copa/Eliminatórias liga {league_id}: apenas {len(df)} jogos após filtro "
            f"(eram {n_antes}). Insuficiente para calibração (mínimo: 8)."
        )

    # Decay temporal agressivo: enfatiza eliminatórias recentes
    if "date" in df.columns:
        df["date"] = pd.to_datetime(df["date"])
        dias_atras = (data_referencia - df["date"]).dt.days.clip(lower=0)
        df["peso"]  = np.exp(-xi * dias_atras)
    ...
    if n_times < 4:
        raise ValueError(f"Copa {league_id}: apenas {n_times} seleções — insuficiente para MLE.")
    ...
    x0 = np.concatenate([
        np.ones(n_times),
        np.ones(n_times),
        np.array([1.10]),   # gamma inicial: campo quase neutro
        np.array([-0.05]),
    ])

    def _constraint_mean(p):
        return np.mean(p[:n_times]) - 1.0

    bounds = (
        [(0.1, 3.0)] * n_times +
        [(0.1, 3.0)] * n_times +
        [(0.9, 1.3)] +          # gamma restrito: campo neutro em WC
        [(-0.35, 0.35)]
    )
    ...
    result = minimize(
        ...
        method="SLSQP",
        bounds=bounds,
        constraints=[{"type": "eq", "fun": _constraint_mean}],
        options={"maxiter": 300, "ftol": 1e-6},
    )

    if not result.success:
        print(f"[motor.py] AVISO: MLE Copa não convergiu (liga {league_id}): {result.message}")
    ...
    # Shrinkage do rho — mais agressivo com n pequeno
    _n = len(df)
    rho *= _n / (_n + 100.0)

    # ── Prior Bayesiano por Ranking FIFA ──────────────────────────────────────
    # Blending: alpha_final = peso_mle * alpha_mle + peso_prior * alpha_prior
    # Quanto menos jogos, mais peso vai para o prior de ranking.
    jogos_por_time = pd.concat([df["home_id"], df["away_id"]]).value_counts().to_dict()
    times_dict: dict = {}

    for t, i in idx_map.items():
        n_j   = int(jogos_por_time.get(t, 0))
        a_mle = float(alphas[i])
        b_mle = float(betas[i])

        if fifa_rankings and t in fifa_rankings:
            ranking        = int(fifa_rankings[t])
            a_prior, b_prior = _prior_por_ranking(ranking)
            forca          = forca_prior
        else:
            a_prior, b_prior = 1.0, 1.0       # prior neutro sem ranking
            forca          = forca_prior * 0.5  # prior mais fraco sem dado externo

        peso_mle   = n_j / (n_j + forca)
        peso_prior = 1.0 - peso_mle
        a_final    = peso_mle * a_mle + peso_prior * a_prior
        b_final    = peso_mle * b_mle + peso_prior * b_prior

        times_dict[int(t)] = {
            "alpha":   float(np.clip(a_final, 0.1, 3.0)),
            "beta":    float(np.clip(b_final, 0.1, 3.0)),
            "n_jogos": n_j,
        }

    media_liga = float(
        (df["home_goals"].sum() + df["away_goals"].sum()) / (2 * max(len(df), 1))
    )
